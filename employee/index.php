...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
            ...
            $sql = "SELECT * FROM employees WHERE email_address = ?";
            ...
            if ($result->num_rows === 1) {
                ...
                if (password_verify($password, $user['password'])) {
                    ...
                    $update_last_login_sql = "UPDATE employees SET last_login = NOW() WHERE id = ?";
                    ...
                    if (isset($_POST['remember'])) {
                        try {
                            ...
                            $stmt = $conn->prepare("INSERT INTO employee_remember_tokens (employee_id, token_hash, expiry) VALUES (?, ?, ?)");
                            if (!$stmt) {
                            ...
                            setcookie('employee_remember_token', $token, time() + (86400 * 30), "/", "", true, true);
                        } catch (Exception $e) {
...
<body>
    <div class="wrapper">
        <form action="" method="POST" novalidate>
            ...
            <div class="register">
                <p>Don't have an account? <a href="http://localhost:8000/">Return To Home Page</a></p>
            </div>
        </form>
    </div>
