...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM customers WHERE reset_token = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $token_expiration = $customers['token_expiration'];
        ...
        if (strtotime($token_expiration) > time()) {
            if ($_SERVER['REQUEST_METHOD'] == 'POST') {
                ...
                $updateStmt = $conn->prepare("UPDATE customers SET password = ?, reset_token = NULL, token_expiration = NULL WHERE reset_token = ?");
...
<body>
<div class="container">
    <h2>Customer Reset Your Password</h2>
    <?php if ($message): ?>
